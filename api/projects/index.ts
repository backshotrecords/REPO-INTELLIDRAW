          ...
          if (rootIds.length > 0) {
            ...
            for (const share of shareRows) {
              ...
              if (
                ...
                shareByRoot.set(share.project_id, share);
              }
            }
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (parentId) {
        ...
        if (!hasCapability(parentAccess, "project.create_folder")) {
          return res.status(403).json({ error: "You do not have permission to add folders here" });
        }
