...
async function loadActiveSkillText(userId: string, canvasId: string): Promise<string> {
  ...
  return "\n\nACTIVE SKILL NOTES (also follow these preferences):\n" +
    skills.map((skill, i) => `${i + 1}. ${skill.title}: ${skill.instruction_text}`).join("\n");
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (canvasId) {
      try {
        skillText = await loadActiveSkillText(authPayload.userId, canvasId);
      } catch { /* non-fatal */ }
    }
...
IMPORTANT: If the code contains a %% EXTERNAL CONTEXT block at the top, preserve that entire block exactly and keep it above %% OBJECTIVES.
